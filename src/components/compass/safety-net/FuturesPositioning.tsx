  ...
  if (!expanded) {
    return (
      <div className="flex items-center gap-1.5 whitespace-nowrap">
        <span className="text-[10px] font-semibold text-slate-400">Futures</span>
        <span className="text-[10px] font-bold" style={{ color }}>{riskLabel}</span>
        {/* Mini dual gauge */}
        <div className="flex items-center gap-0.5">
          <div className="w-4 h-2 bg-slate-800 rounded-full overflow-hidden">
  ...
  return (
    <div className="p-3 w-64">
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-slate-300">Futures Positioning</span>
      ...
      <div className="space-y-3 mb-3">
        <div>
          ...
          <div className="relative h-2.5 bg-slate-800 rounded-full overflow-hidden">
            {/* Threshold markers */}
            <div className="absolute top-0 h-full w-px bg-slate-600/50" style={{ left: '50%' }} />
            <div className="absolute top-0 h-full w-px bg-slate-600/50" style={{ left: '70%' }} />
            <div
        ...
        <div>
          <div className="flex justify-between text-[10px] mb-0.5">
            <span className="text-slate-400">Stretching</span>
      ...
      <div className="border-t border-slate-700/50 pt-2 space-y-1.5">
        <div className="flex justify-between text-[10px]">
          <span className="text-slate-500">Net Speculative</span>
          <span className="font-mono font-bold" style={{ color: netSpec > 0 ? '#22C55E' : '#EF4444' }}>
            {netSpec > 0 ? '+' : ''}{netSpec.toLocaleString()}
          </span>
        </div>
        <div className="flex justify-between text-[10px]">
          <span className="text-slate-500">Commercial Hedging</span>
