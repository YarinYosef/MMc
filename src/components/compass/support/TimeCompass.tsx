  ...
  if (!expanded) {
    return (
      <div className="flex items-center gap-1 whitespace-nowrap">
        ...
        <span className="text-[9px] font-bold" style={{ color: timeColor }}>{timeLabel}</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-3 w-64">
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-slate-300">Time Compass</span>
      ...
      <div className="border-t border-slate-700/50 pt-2">
        <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-1.5 font-semibold">Calendar Events</div>
    ...
    <div>
      <div className="flex justify-between text-[10px] mb-0.5">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[7px] text-slate-600">({description})</span>
        </div>
...
function CalendarItem({ label, value, highlight = false, danger = false }: { label: string; value: string; highlight?: boolean; danger?: boolean }) {
  ...
  return (
    <div className="bg-slate-800/30 rounded px-2 py-1">
      ...
      <div className="text-[10px] font-bold" style={{ color: textColor }}>{value}</div>
    </div>
  );
}
