  ...
  return (
    <svg width={size} height={size / 2 + 2} className="shrink-0">
      ...
      <path
        ...
        strokeLinecap="round"
      />
      <text x={cx} y={cy - 1} textAnchor="middle" fontSize="6" fill="#94A3B8" fontFamily="monospace">
        {label}
      </text>
    </svg>
  );
  ...
  if (!expanded) {
    return (
      <div className="flex items-center gap-1 whitespace-nowrap">
        <span className="text-[10px] font-semibold text-slate-400">VIX</span>
  ...
  return (
    <div className="p-3 w-80">
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-slate-300">VIX Compass</span>
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-slate-500">Combined Stress</span>
      ...
      <div className="border-t border-slate-700/50 pt-2">
        <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-1.5 font-semibold">Term Structure</div>
        <div className="flex items-end justify-between px-2">
          {/* 9D bar */}
          <div className="flex flex-col items-center gap-0.5">
            <div className="text-[10px] font-mono text-slate-300">{vix9d.toFixed(1)}</div>
            <div
            ...
            />
            <div className="text-[8px] text-slate-500">9D</div>
          </div>
          {/* Spot bar */}
          <div className="flex flex-col items-center gap-0.5">
            <div className="text-[10px] font-mono font-bold text-slate-200">{spotVix.toFixed(1)}</div>
            <div
              className="w-8 rounded-t transition-all duration-500 border border-slate-600"
              style={{
                height: `${Math.max((spotVix / 50) * 40, 4)}px`,
                backgroundColor: stressColor((spotVix / 50) * 100),
              }}
            />
            <div className="text-[8px] text-slate-400 font-semibold">Spot</div>
          </div>
          {/* 3M bar */}
          <div className="flex flex-col items-center gap-0.5">
            <div className="text-[10px] font-mono text-slate-300">{vix3m.toFixed(1)}</div>
            <div
              className="w-6 rounded-t transition-all duration-500"
              style={{
            ...
            />
            <div className="text-[8px] text-slate-500">3M</div>
          </div>
        </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-0.5">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] px-1 py-0.5 rounded" style={{ color: statusColor, backgroundColor: `${statusColor}15` }}>
            {status}
          </span>
          <span className="text-[10px] font-mono font-bold text-slate-200">{value.toFixed(1)}</span>
        </div>
      </div>
      {/* Progress bar with threshold markers */}
      <div className="relative h-2 bg-slate-800 rounded-full overflow-hidden mb-0.5">
        <div
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          {thresholdLabels.map((tl, i) => (
            <span key={i} className="text-[7px] text-slate-600">{tl}</span>
          ))}
        </div>
        <span className="text-[8px]" style={{ color: momColor }}>
