  ...
  if (!expanded) {
    return (
      <div className="flex items-center gap-1.5 whitespace-nowrap">
        <span className="text-[10px] font-semibold text-slate-400">MSFT</span>
        <MiniSparkline data={priceData} color={gapColor} />
        <span className="text-[10px] font-mono text-slate-300">{athDist.toFixed(1)}%</span>
        <span className="text-[9px] font-bold px-1 rounded" style={{ color: breakoutColor, backgroundColor: `${breakoutColor}15` }}>
          {breakout}
        </span>
      </div>
    );
    ...
    <div className="p-3 w-80">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-300">Microsoft Proxy</span>
          <span className="text-[8px] text-slate-600">MEGA-CAP HEALTH</span>
        </div>
        <span
      ...
      <div className="flex items-baseline gap-3 mb-2">
        <span className="text-xl font-mono font-bold text-slate-100">${price.toFixed(2)}</span>
        <div className="flex items-center gap-1">
          <span className="text-[11px] font-mono" style={{ color: gapColor }}>
            {priceChange >= 0 ? '+' : ''}{priceChange.toFixed(2)}
          </span>
          <span className="text-[10px] font-mono px-1 rounded" style={{ color: gapColor, backgroundColor: `${gapColor}15` }}>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-[9px] mb-0.5">
          <span className="text-slate-500">Distance from ATH (${ath})</span>
        ...
        <div className="relative h-2.5 bg-slate-800 rounded-full overflow-hidden">
          ...
          />
          {/* ATH marker */}
          <div className="absolute right-0 top-0 h-full w-0.5 bg-slate-500" />
        </div>
      ...
      {/* Price chart */}
      {priceData.length > 2 && (
        <div className="mb-3 bg-slate-800/30 rounded p-2">
          <div className="text-[8px] text-slate-500 mb-1">30-Period Price Action</div>
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px]">
        ...
        <div className="bg-slate-800/30 rounded px-2 py-1">
          ...
          <div className="font-mono text-slate-300">{state.confidence}%</div>
        </div>
      </div>
    </div>
  );
...
function PriceChart({ data, color }: { data: number[]; color: string }) {
  ...
  return (
    <svg width={w} height={h} className="w-full" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {/* Grid lines */}
      <line x1="0" y1={h * 0.25} x2={w} y2={h * 0.25} stroke="#334155" strokeWidth="0.5" strokeDasharray="2,3" />
      <line x1="0" y1={h * 0.5} x2={w} y2={h * 0.5} stroke="#334155" strokeWidth="0.5" strokeDasharray="2,3" />
      <line x1="0" y1={h * 0.75} x2={w} y2={h * 0.75} stroke="#334155" strokeWidth="0.5" strokeDasharray="2,3" />
      {/* Area fill */}
