  ...
  if (!expanded) {
    return (
      <div className="flex items-center gap-1.5 whitespace-nowrap">
        <span className="text-[11px] font-semibold text-[#999999]">$Liq</span>
        <span className="text-[11px] font-bold" style={{ color }}>{liqState}</span>
        <span className="text-[10px] px-1 rounded" style={{ color: biasColor, backgroundColor: `${biasColor}15` }}>
          {bias}
        </span>
        {/* Mini trend arrow */}
  ...
  return (
    <div className="p-3 w-80">
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-[#999999]">Dollar & Global Liquidity</span>
      ...
      {/* Trend & Compression */}
      <div className="bg-white/[0.03] rounded px-2 py-1.5 mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#777777]">Compression</span>
            <span className="text-[11px] font-mono text-[#999999]">{compression}%</span>
          </div>
        </div>
        {/* Compression bar */}
      ...
      <div className="border-t border-white/[0.08] pt-2">
        <div className="text-[10px] text-[#777777] mb-1.5 font-semibold uppercase tracking-wider">USD Cross-Asset Correlation</div>
          ...
          {corrItems.map(({ label, val, abbr }) => {
            ...
            return (
              <div key={label} className="bg-white/[0.03] rounded px-1.5 py-1">
                <div className="text-[9px] text-[#777777] mb-0.5">{abbr}</div>
                <div className="text-[11px] font-mono font-bold mb-0.5" style={{ color: c }}>
                  {val > 0 ? '+' : ''}{val.toFixed(2)}
                </div>
                <div className="h-1 bg-white/[0.06] rounded-full overflow-hidden">
                  <div
  ...
  return (
    <div className="bg-white/[0.03] rounded px-2 py-1.5">
      <div className="flex items-center justify-between">
        <div className="text-[10px] text-[#777777]">{label}</div>
      ...
      <div className="text-[7px] text-[#777777]">{sublabel}</div>
    </div>
  );
}
