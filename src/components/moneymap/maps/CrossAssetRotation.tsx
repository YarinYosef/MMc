...
export function CrossAssetRotation() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="text-[10px] text-slate-400 px-2 pb-1">Capital flow across asset classes</div>
      ...
      <div className="flex gap-3 px-2 py-1 border-t border-slate-700/50">
        {data.map((d) => (
          <div key={d.etf} className="flex items-center gap-1">
            ...
            <span className="text-[9px] text-slate-400">{d.etf}</span>
            <span className={`text-[9px] ${d.momentum >= 0 ? 'text-green-400' : 'text-red-400'}`}>
