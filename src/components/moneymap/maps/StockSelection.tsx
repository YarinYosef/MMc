...
export function StockSelection() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="text-[10px] text-slate-400 px-2 pb-1">Top ranked candidates by composite score</div>
        ...
        <table className="w-full text-[9px]">
          ...
          <tbody>
            {ranked.map((r, i) => (
              ...
              >
                <td className="py-1 px-1.5 text-slate-500">{i + 1}</td>
                <td className="py-1 px-1.5">
                  <div className="text-slate-200 font-semibold">{r.symbol}</div>
                  <div className="text-slate-500 text-[8px]">{r.sector}</div>
                </td>
                <td className="py-1 px-1.5 text-right text-slate-200">${r.price.toFixed(2)}</td>
                <td className={`py-1 px-1.5 text-right ${r.changePercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {formatPercent(r.changePercent)}
                </td>
                <td className="py-1 px-1.5 text-right text-slate-400">{formatVolume(r.volume)}</td>
