...
import watchlistData from '@/json/watchlists.json';

const DEFAULT_GROUPS: WatchlistGroup[] = watchlistData.groups.map((g, i) => ({
  id: g.id,
  name: g.name,
  color: g.color || WATCHLIST_COLORS[i % WATCHLIST_COLORS.length],
  items: g.items.map((symbol) => ({
    symbol,
